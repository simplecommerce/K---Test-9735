...
const { 
  ...
  FiChevronRight, FiLogOut, FiRefreshCw, FiLoader
} = FiIcons;

const Sidebar = ({ activeTab, setActiveTab }) => {
  const { user, logout, refreshUserProfile, profileLoading } = useAuth();
  ...
  // Define menu items with permission checks based on current user role
  const getMenuItems = () => {
    const items = [
      {
        id: 'agents',
    ...
    // Add team management for administrators only
    // This check uses the current user role from the profile
    if (user && user.role === 'Administrator') {
      console.log('Adding team management menu item for administrator');
      items.push({
        id: 'team-management',
        icon: FiUsers,
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full transition-all duration-300 relative ${isCollapsed ? 'w-20' : 'w-64'}`}>
      ...
      {/* User Role Debug Info - Only show in development */}
      {process.env.NODE_ENV === 'development' && !isCollapsed && user && (
        <div className="px-4 py-2 bg-gray-100 dark:bg-gray-800 text-xs">
          <div className="text-gray-600 dark:text-gray-400">
            Role: <span className="font-mono text-blue-600 dark:text-blue-400">{user.role}</span>
          </div>
          <div className="text-gray-600 dark:text-gray-400">
            ID: <span className="font-mono text-gray-500">{user.id?.slice(0, 8)}...</span>
          </div>
        </div>
      )}
      ...
      <div className="px-4 py-6 border-t border-gray-200 dark:border-gray-700">
        <div className="space-y-3">
          ...
          >
            <SafeIcon 
              icon={isRefreshing || profileLoading ? FiLoader : FiRefreshCw} 
              className={`w-5 h-5 text-gray-500 dark:text-gray-400 ${(isRefreshing || profileLoading) ? 'animate-spin' : ''}`} 
            />
            {!isCollapsed && (
              <span className="ml-3 font-medium">
                {isRefreshing || profileLoading ? 'Refreshing...' : 'Refresh Profile'}
              </span>
            )}
          </motion.button>
